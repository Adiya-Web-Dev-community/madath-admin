      ...
      <table className="w-full text-center">
        <thead className="bg-gray-50">
          <tr>
            ...
            <th className="p-2">View Search Activity</th>
        ...
        {!isLoading && (
          <tbody>
            {data?.data?.map((user: UserLoginListType) => (
              <tr key={user._id}>
                <td className="p-2">{user.fullName}</td>

                <td className="p-2">{user.email}</td>
                <td className="p-2">
                  <button onClick={()=>{navigate('/Admin/Client/Search-Activity/'+user._id)}} className="p-1 rounded bg-emerald-500 text-white"><EyeIcon width="25px"/></button>
                </td>
