...
import { DeleteIcon, EyeIcon, UpdateIcon } from "../../component/icon/Icon";
import { AdminLoginUserListType } from "../../types/page";
...
const LoginUsers = () => {
  ...
  return (
    ...
    <div className="w-full h-14  flex   rounded-md  ">
       <button onClick={()=>{setRightsVal('Co Admin ')}} 
       className={`rounded-tl-md duration-500 ${rightsVal==='Co Admin '?'bg-emerald-500 text-white':'bg-white text-black'} w-56 `}  >
        Co Admin Login
       </button>
       <button onClick={()=>{setRightsVal('admin')}} 
       className={`rounded-tr-md duration-500 ${rightsVal==='admin'?'bg-emerald-500 text-white':'bg-white text-black'} w-56 `}>
        Super Admin Login   
       </button>
      ...
      <div className="w-full py-4 px-2 flex flex-col ">
        <div className="w-full py-4 px-2 flex justify-between">
          <div>
            ...
            >
              Add  {rightsVal==='admin' ? "Admin" : "Co Admin "} login
            </button>
        </div>
      </div>
      ...
      <table className="w-full text-center ">
        <thead className="bg-gray-50 ">
          <tr>
           ...
           {rightsVal==='Co Admin '&& <th className="p-2">View Activity</th>}
        ...
        {!isLoading && (
          <tbody>
            {
            data?.data?.map((user: AdminLoginUserListType) => (
              <tr key={user._id}>
                ...
                <td className="p-2">
                  <button
                    className={`py-1 px-4 border  text-sm font-[400] rounded-xl ${
                      user.role==='admin' ? "bg-emerald-500 text-white " : "bg-orange-500 text-white"
                    }`}
                  >
                ...
                {rightsVal==='Co Admin '&& <td className="p-2">
                 <button 
                 onClick={()=>postMutationToCheckUserRole.mutate({email:user.email,userId:user._id})} 
                 className=" bg-emerald-500 text-white p-1.5 rounded"> <EyeIcon width="20px"/> </button>                   
                </td>}
